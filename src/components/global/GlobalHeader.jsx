// src/components/global/GlobalHeader.jsx
import React, { useState } from 'react';
import PropTypes from 'prop-types';
import useGlobalStore from '../../store/globalStore.js';
import EchoShop from './EchoShop.jsx';
    ...
    <>
      <header className="global-header" style={{
        ...
        <div className="header-left" style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          />
          <span className="username" style={{ fontWeight: 600, color: 'var(--primary)', fontSize: '1.1rem' }}>
            {user?.username || 'Guest'}
          </span>
          {user?.level && (
            <span className="user-level" style={{ color: 'var(--secondary)', fontSize: '0.95rem', marginLeft: 4 }}>
              Lv. {user.level}
            </span>
          )}
        </div>
        ...
        <div className="header-right" style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <button
            className="echo-chip cartoon-button"
            onClick={() => setShowShop(true)}
            title="Open Echo Shop"
      ...
      {showShop && <EchoShop onClose={() => setShowShop(false)} />}
    </>
  );
}
